                ...
                for pass_idx in range(len(locs) + 1):  # +1 for being inside taxi
                    for dest_idx in range(len(locs)):
                        ...
                        for action in range(num_actions):
                            ...
                            taxi_loc = (row, col)
                            # 0-1 --> 1,3 
                            if action == 0: #and (self.desc[2 + row , 2 * col] != b"|"): #move south
                                new_row = min(row + 1, max_row)
                                if self.desc[1 + new_row , 2 * col] == b"|":
                                    reward = -10
                            elif action == 1: #and (self.desc[row, 2 * col] != b"|"): #move north
                                new_row = max(row - 1, 0)
                                if self.desc[1 + new_row, 2 * col] == b"|":
                                    reward = -10
                            if action == 2 and self.desc[1 + row, 2 * col + 2] == b":": #and self.desc[1 + row, 2 * col + 3] != b"O": #move east
                                new_col = min(col + 1, max_col)
                            elif action == 3 and self.desc[1 + row, 2 * col] == b":": #and self.desc[1 + row, 2 * col - 1] != b"O":#move west
                                new_col = max(col - 1, 0)
